...
const homeTitle = '爱窝啦·货源雷达 | AI账号卖家货源、渠道核价与经营日报';
const homeDescription = '为 AI 账号卖家聚合公开货源、渠道报价、官方地区价格、库存变化和账号商机日报，帮助卖家核验货源、测算利润并安排当天经营动作。';

export const metadata: Metadata = {
  title: homeTitle,
  description: homeDescription,
  keywords: ['AI账号货源', 'AI账号卖家', 'AI账号进货', 'AI账号利润', 'AI账号商机日报', 'ChatGPT货源', 'Claude货源'],
...
const faqItems = [
  { question: '卖家怎样最快找到 ChatGPT Plus 货源？', answer: '进入订阅货源，ChatGPT 默认排在分类首位。同一标准商品内先显示可采购报价，再比较价格、库存、更新时间和售后说明。' },
  { question: '为什么目录最低价不能直接当进货成本？', answer: '低价可能对应共享、短保、低库存或很久未更新的商品。接单前至少核验交付类型、库存、更新时间、原始链接和售后边界。' },
  { question: '卖家怎样使用官方价格和第三方货源？', answer: '官方价格用于判断基准成本、地区与周期差价，第三方货源用于核对实际进货选择。第三方低价不代表官方授权或风险更低。' },
  { question: '账号商机日报怎样使用实时货源？', answer: '日报把需求信号与当前可采购报价、库存、价差和停止条件放在一起。卖家可以进入商品页核验货源，再用利润计算器测算。' },
];
...
const modules = [
  { href: '/card-products', icon: Database, title: '第三方账号货源', text: '标准商品聚合、库存优先、原始报价可核验。' },
...
export default async function HomePage() {
  ...
  const statItems = [
    ...
    ['当前可采购报价', stats.offers.toLocaleString('zh-CN')],
    ...
    <main className="market-page">
      ...
      <section className="market-shell grid gap-7 pb-14 pt-12 sm:pb-20 sm:pt-20 lg:grid-cols-[minmax(0,1.25fr)_minmax(20rem,0.75fr)] lg:items-end">
        <div>
          ...
          <h1 className="market-display mt-6 max-w-4xl text-[clamp(2.7rem,7vw,5.8rem)]">找货、核价、<br />算利润</h1>
        </div>
        <div className="pb-1">
          <p className="max-w-2xl text-base leading-8 text-gray-600 sm:text-lg">为 AI 账号卖家整理公开货源。先找到可采购、可复核的商品，再核验库存、成本、售后与利润。</p>
      ...
      <section className="market-shell pb-14" aria-labelledby="choose-path">
        <div className="mb-5 flex items-end justify-between gap-4">
          ...
          <span className="hidden text-sm text-gray-500 sm:block">两条路径都使用同一份实时货源数据</span>
        </div>
        <div className="grid gap-4 lg:grid-cols-12">
          {entryPaths.map((item, index) => {
            const Icon = item.icon;
            return (
              <Link key={item.href} href={item.href} className="market-card group flex flex-col p-6 transition-colors hover:border-emerald-400 sm:p-7 lg:col-span-6 lg:min-h-[18rem]">
      ...
      <section className="border-y border-gray-200 bg-white/75 py-12 sm:py-16">
        <div className="market-shell">
          <div className="grid gap-8 lg:grid-cols-[0.8fr_1.2fr] lg:items-end">
            <div><span className="radar-kicker">Live marketplace</span><h2 className="market-display mt-3 text-3xl sm:text-4xl">数据规模不等于可用，<br />可采购才排在前面</h2><p className="mt-4 max-w-lg text-sm leading-7 text-gray-600">目录保留完整商品，但分类内优先展示有库存、有有效报价的项目。无法确认的数据不会伪装成零元或有货。</p></div>
      ...
      <section className="market-shell grid gap-8 py-14 lg:grid-cols-[0.8fr_1.2fr] lg:py-16">
        ...
        <ol className="market-card divide-y divide-gray-200 px-5 sm:px-7">
          {[
            ...
            [Database, '核验货源', '打开关联商品，确认可采购报价、库存、更新时间和原页。'],
            [Calculator, '测算利润', '加入支付、退款、售后、获客成本，得到保本价。'],
            [Code2, '设置停止条件', '库存失效、价格倒挂或证据过期时立即暂停。'],
          ].map(([Icon, title, text], index) => {
            const StepIcon = Icon as typeof Bot;
            return <li key={String(title)} className="grid grid-cols-[2.5rem_1fr] gap-3 py-5 sm:grid-cols-[2.5rem_8rem_1fr] sm:items-center"><span className="flex h-9 w-9 items-center justify-center rounded-full bg-gray-100 text-gray-700"><StepIcon className="h-4 w-4" /></span><strong className="text-sm text-gray-950"><span className="mr-2 font-mono text-xs text-gray-400">0{index + 1}</span>{String(title)}</strong><span className="col-start-2 text-sm leading-6 text-gray-600 sm:col-start-auto">{String(text)}</span></li>;
          })}
        </ol>
      </section>
      ...
      <section className="border-y border-gray-200 bg-white py-12">
        <div className="market-shell grid gap-8 lg:grid-cols-[0.75fr_1.25fr]">
          ...
          <div className="grid gap-5 text-sm leading-7 text-gray-600 sm:grid-cols-2"><p>本站聚合公开页面，不代收货款、不保管账号、不替第三方交易背书。采购或接单前必须打开原始商品页复核。</p><p>价格、库存、交付和售后可能变化。暂无可靠报价时明确显示“暂无报价”，不会用无关商品或虚构数据补位。</p></div>
        </div>
      </section>

      <section className="market-shell py-14">
        <div className="flex flex-wrap items-end justify-between gap-4"><div><span className="radar-kicker">FAQ</span><h2 className="market-display mt-2 text-3xl">进货和接单前先问清楚</h2></div><Link href="/guide" className="text-sm font-bold text-blue-700 hover:underline">阅读卖家核验指南 →</Link></div>
